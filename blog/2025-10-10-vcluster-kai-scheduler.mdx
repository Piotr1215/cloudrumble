...
graph TB
    ...
    subgraph "Host Cluster"
        RES["Synced Resources<br/>(Pods, Services, PVCs,<br/>ConfigMaps, Secrets, etc.)"]
    end
...
graph LR
    subgraph "vCluster"
        VAPI["vCluster<br/>API Server"]
    end

    SYNCER["Syncer<br/>(bidirectional)"]

    subgraph "Host Cluster"
        HAPI["Host<br/>API Server"]
        RES["Various Resources<br/>(Pods, Services, PVCs,<br/>ConfigMaps, Secrets, etc.)"]
    end

    VAPI -->|"1. Create Resource"| SYNCER
    SYNCER -->|"2. Transform & Apply"| HAPI
    HAPI -->|"3. Create in Host"| RES
    RES -.->|"4. Status Updates"| SYNCER
    SYNCER -.->|"5. Sync Status"| VAPI

    style SYNCER fill:#51cf66,color:#fff
    style VAPI fill:#4dabf7,color:#fff
    style HAPI fill:#868e96,color:#fff
```
...
![vCluster Isolation](_media/03-solution-vcluster-isolation.png)
