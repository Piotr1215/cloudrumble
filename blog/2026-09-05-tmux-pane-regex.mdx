...
Up walks to an older occurrence, Down to a newer one. Enter or Tab pastes. Ctrl+Y copies the selection to the clipboard instead, through tmux's OSC 52 path, so no clipboard helper is needed. Esc closes the popup and leaves the pane as it was.
