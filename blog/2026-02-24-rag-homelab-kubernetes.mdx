...
The core of the system is a Python script that Claude Code runs as a `UserPromptSubmit` hook. It fires only when you end a prompt with `:rag`. The hook searches both knowledge stores in parallel and prints the results to stdout, which Claude Code injects as context.
