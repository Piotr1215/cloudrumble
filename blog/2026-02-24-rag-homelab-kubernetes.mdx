...
description: "R2R on Kubernetes, pgvector for storage, Ollama for embeddings, and a Python hook that searches your knowledge on every prompt in under 400ms."
---
...
AI agents work with what you give them. The system prompt, the files you open, the tools you configure. But they don't know what you wrote in your Obsidian vault last week. They don't remember the debugging pattern you discovered three sessions ago.

You can manually paste context. You can tell the agent "search my notes for X." But that assumes you know what's relevant before you ask the question. The whole point of accumulated knowledge is that connections surface when you need them, not when you predict them.

What if every prompt automatically searched your knowledge stores and injected the relevant context before the agent saw it? Not a tool the agent calls. A hook that fires before the agent thinks.

I built this on my homelab Kubernetes cluster. R2R as the RAG backend, pgvector for vector storage, Ollama on a desktop for embeddings, and a 196-line Python hook that searches two knowledge sources in parallel on every prompt. Total latency: under 400ms.
...
3. The hook searches R2R (Obsidian vault, ~200ms) and agent-memory (session insights, ~400ms) in parallel
...
|-----------|---------|---------|---------|
| R2R (SciPhi) | Obsidian vault search | pgvector on K8s, 10Gi PVC | ~200ms |
| agent-memory | Session memory search | Redis Stack on K8s, 2Gi PVC | ~400ms |
...
This is pure linear algebra. Dot products and normalization. That's why it runs in 200ms with zero token costs.
...
The core of the system is a Python script that Claude Code runs as a `UserPromptSubmit` hook. Every non-trivial prompt triggers it. The hook searches both knowledge stores in parallel and prints the results to stdout, which Claude Code injects as context.
...
The script is 217 lines of stdlib Python. No dependencies beyond what ships with Python 3. Four design decisions matter.

**Skip trivial prompts.** "yes", "ok", "push", "continue" — these don't need context. A regex filter catches them. Prompts under 30 characters get skipped too. About 40% of prompts during a typical session are trivial.

```python
TRIVIAL = re.compile(
    r"^(yes|no|ok|okay|y|n|sure|thanks|done|continue|"
    r"go ahead|lgtm|sounds good|perfect|great|good)$",
    re.IGNORECASE,
)
```

**Only fire for questions and tasks.** Even non-trivial prompts like "refactor the auth module" don't always need RAG context. A second regex gates the hook to prompts that look like questions or actionable tasks — words like "how", "configure", "deploy", "troubleshoot". This cuts unnecessary searches by another ~30%.

```python
QUESTION_SIGNAL = re.compile(
    r"(how|what|why|where|when|which|can|does|is there|explain|"
    r"troubleshoot|configure|setup|deploy|create|add|remove|"
    r"fix|debug|install|upgrade|migrate|backup|restore)",
    re.IGNORECASE,
)
```

**Parallel search with strict timeouts.** R2R takes ~200ms, agent-memory takes ~400ms (sometimes 3s on a cold start). Both run in a `ThreadPoolExecutor` with a 2-second hard ceiling. If agent-memory times out, R2R results still arrive. If both fail, the hook stays silent.

```python
with ThreadPoolExecutor(max_workers=2) as pool:
    futures = {
        pool.submit(search_r2r, prompt): "r2r",
        pool.submit(search_memory, prompt): "memory",
    }
...
**Minimum relevance score.** R2R chunks below 0.60 similarity get dropped. This prevents low-quality matches from consuming context window tokens. The threshold started at 0.45 and moved up after observing too many marginal hits adding noise.
...
The hook adds about 400ms to each prompt. A brief pause before the agent responds. In practice, negligible — the agent's thinking time dwarfs the hook latency.

| Metric | Value |
|--------|-------|
| R2R search latency | ~200ms |
| Agent-memory search latency | ~400ms |
| Total hook latency (parallel) | ~400ms |
| Obsidian notes indexed | 392 |
| Agent memories searchable | ~2,800 |
| Prompts skipped (trivial) | ~40% |
