...
**Parallel search with generous timeouts.** R2R takes ~200ms, agent-memory takes ~400ms (sometimes longer on a cold start). Both run in a `ThreadPoolExecutor` with per-request and total timeout ceilings. Timeouts are generous — better to wait a beat than lose context. If one source times out, the other's results still arrive. If both fail, the hook stays silent.
